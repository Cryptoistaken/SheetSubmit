...
// ponytail: web site calls the backend directly (the CF proxy strips gzip via
// accept-encoding:identity and swaps error JSON for HTML pages). Android WebView
...
export const api = {
  ...
  pageAdvanced: (cookie: string) =>
    request<{ eligible?: boolean; error?: string | null; banReason?: string | null; pageName?: string | null; linkedNumber?: string | null }>("/fb/page-advanced", {
      method: "POST",
      body: JSON.stringify({ cookie }),
    }),
  fbCheck: (uids: string[]) =>
  ...
  setPoolFlags: (combos: Record<string, boolean>) =>
    request<PoolFlags>("/pools/flags", { method: "PUT", body: JSON.stringify({ combos }) }),
