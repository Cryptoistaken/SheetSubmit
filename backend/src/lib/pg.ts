...
export async function closeDatabase() { await db.end({ timeout: 5 }); }
...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "register": await db`INSERT INTO file_index(file_id,owner_id,archived,data) VALUES(${a.file.id},${a.uid},false,${db.json(a.file)}) ON CONFLICT(file_id) DO UPDATE SET owner_id=EXCLUDED.owner_id,archived=false,data=EXCLUDED.data,updated_at=now()`; return { ok: true };
    ...
    case "archive": await db`UPDATE file_index SET archived=${!!a.archived},data=${db.json(a.file)},updated_at=now() WHERE file_id=${a.id}`; return { ok: true };
    case "batchArchive": await db.begin(async (tx: any) => { for (const f of a.files as SheetFile[]) await tx`UPDATE file_index SET archived=false,data=${tx.json(f)},updated_at=now() WHERE file_id=${f.id}`; }); return { ok: true };
    ...
    case "metaSet": await db`INSERT INTO meta(k,v) VALUES(${a.k},${db.json(a.v)}) ON CONFLICT(k) DO UPDATE SET v=EXCLUDED.v`; return { ok: true };
...
async function deviceOp(kind: string, a: any) {
  ...
  return db.begin(async (tx: any) => {
    ...
    if (kind === "delete") await tx`DELETE FROM meta WHERE k=${didKey}`; else { await tx`INSERT INTO meta(k,v) VALUES(${didKey},${tx.json({ chatId: a.chatId })}) ON CONFLICT(k) DO UPDATE SET v=EXCLUDED.v`; await tx`INSERT INTO meta(k,v) VALUES(${chatKey},${tx.json({ did: a.did })}) ON CONFLICT(k) DO UPDATE SET v=EXCLUDED.v`; }
    return { ok: true };
  });
}

async function fileOp(id: string, op: string, a: any) {
  if (op === "init") return db.begin(async (tx: any) => { await tx`INSERT INTO file_meta(file_id,data,seq) VALUES(${id},${tx.json(a.file)},0) ON CONFLICT(file_id) DO UPDATE SET data=EXCLUDED.data,seq=0`; await tx`DELETE FROM file_rows WHERE file_id=${id}`; await tx`DELETE FROM file_logs WHERE file_id=${id}`; for (const [i, r] of (a.rows || []).entries()) await tx`INSERT INTO file_rows(file_id,idx,data) VALUES(${id},${i},${tx.json(r)})`; return { ok: true }; });
  ...
  if (op === "wipe") return db.begin(async (tx: any) => { const rows = (await tx`SELECT data FROM file_rows WHERE file_id=${id}`).map((r: any) => r.data); await tx`DELETE FROM file_meta WHERE file_id=${id}`; return { ok: true, rows }; });
  ...
  return db.begin(async (tx: any) => {
    ...
    const next = current + 1; await tx`UPDATE file_meta SET data=${tx.json(file)},seq=${next} WHERE file_id=${id}`; await tx`DELETE FROM file_rows WHERE file_id=${id}`; for (const [i, r] of rows.entries()) await tx`INSERT INTO file_rows(file_id,idx,data) VALUES(${id},${i},${tx.json(r)})`; await tx`INSERT INTO file_logs(file_id,ts,action,seq) VALUES(${id},${Date.now()},${String(a.action || (op === "append" ? "append" : "edit"))},${next})`; await tx`DELETE FROM file_logs WHERE file_id=${id} AND id NOT IN (SELECT id FROM file_logs WHERE file_id=${id} ORDER BY id DESC LIMIT 200)`;
...
async function transition(password: string, op: string, a: any) { return db.begin(async (tx: any) => { const d: any = (await tx`SELECT * FROM downloads WHERE password=${password} AND id=${a.id} FOR UPDATE`)[0]; if (!d) throw new Error("not found"); const keys = json(d.keys) as string[], now = Date.now(); if (["holdApprove"].includes(op)) { if (d.status !== "HOLD") throw new Error(d.status === "APPROVED" ? "already approved" : "not a hold"); for (const k of keys) { await tx`UPDATE pool_rows SET state='claimed',hold_id=NULL,claimed_by=${d.claimed_by || a.uid},claimed_at=${now} WHERE password=${password} AND pool_id=${d.pool_id} AND row_key=${k} AND state='held' AND hold_id=${d.id}`; await tx`INSERT INTO pool_ledger(password,pool_id,row_key,user_id,action,ts) VALUES(${password},${d.pool_id},${k},${a.uid || d.claimed_by},'approve',${now})`; } await tx`UPDATE downloads SET status='APPROVED',reverted=false WHERE password=${password} AND id=${d.id}`; return { ok: true, approved: keys.length, id: d.id, status: "APPROVED" }; }
    if (["holdReject", "holdRevert", "holdReturn"].includes(op) && d.status !== "HOLD") throw new Error(d.status === "REJECTED" ? "already rejected" : d.status === "APPROVED" ? "already approved" : "not a hold"); const action = d.status === "HOLD" ? "reject" : "revert"; for (const k of keys) { await tx`UPDATE pool_rows SET state='available',hold_id=NULL,claimed_by=NULL,claimed_at=NULL WHERE password=${password} AND pool_id=${d.pool_id} AND row_key=${k} AND ${action === "reject" ? tx`state='held' AND hold_id=${d.id}` : tx`TRUE`}`; await tx`INSERT INTO pool_ledger(password,pool_id,row_key,user_id,action,ts) VALUES(${password},${d.pool_id},${k},${a.uid || d.claimed_by},${action},${now})`; } await tx`UPDATE downloads SET reverted=true,status=${action === "reject" ? "REJECTED" : "REVERTED"} WHERE password=${password} AND id=${d.id}`; return { ok: true, reverted: keys.length, ...(action === "reject" ? { rejected: keys.length } : {}), id: d.id, status: action === "reject" ? "REJECTED" : "REVERTED" }; }); }
