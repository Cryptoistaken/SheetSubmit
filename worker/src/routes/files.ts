...
files.put("/:id/append", async (c) => {
  ...
  const feedRows: Row[] = saved.rows ?? []; if (feedRows.length) c.executionCtx.waitUntil(feedPools(c.env, updated, feedRows, c.get("uid"))); else { const rows = await rpc(c.env.FILES, file.id, "rows").catch(() => [] as Row[]) as Row[]; if (rows.length) c.executionCtx.waitUntil(feedPools(c.env, updated, rows, c.get("uid"))); }
...
async function removePoolRows(env: Env, password: string, rows: Row[], uid: string, file?: SheetFile | null) { const preset = file ? resolvePreset(file) : null; const byPool = new Map<string, Set<string>>(); rows.forEach((row) => { const pool = poolForRowWithPreset(row, preset); if (!pool) return; const key = poolId(row); if (key) (byPool.get(pool) || byPool.set(pool, new Set()).get(pool)!).add(key); }); await Promise.all([...byPool].map(([pool, keys]) => rpc(env.POOLS, password, "removeAvailable", { pool, keys: [...keys], uid }).catch((e: any) => console.error("removeAvailable failed", pool, e?.message ?? e)))); }
