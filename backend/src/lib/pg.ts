...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "walletCredit": { const uid = String(a.uid || ""), adminUid = String(a.adminUid || ""); const title = String(a.title || "").trim().slice(0, 128) || "Manual credit"; const amount = Math.round(Number(a.amount) * 100) / 100; if (!uid || !Number.isFinite(amount) || amount <= 0 || amount > 100000) throw new Error("invalid credit"); const now = Date.now(); return db.begin(async (tx: any) => { await tx`INSERT INTO wallets(user_id,balance) VALUES(${uid},0) ON CONFLICT(user_id) DO NOTHING`; const updated: any = await tx`UPDATE wallets SET balance=balance+${amount} WHERE user_id=${uid} RETURNING balance`; const bal = Number(updated[0].balance); const id = crypto.randomUUID(); await tx`INSERT INTO wallet_transactions(id,user_id,type,amount,balance_after,description,meta,created_at) VALUES(${id},${uid},'CREDIT',${amount},${bal},${title},${j({ manual: true, ...(adminUid ? { admin_uid: adminUid } : {}) })},${now})`; return { id, uid, amount, balance: bal, title }; }); }
