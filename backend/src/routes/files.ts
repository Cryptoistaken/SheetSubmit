...
files.post("/:id/restore-snapshot", async (c) => {
  ...
  if ((out.file as SheetFile)?.password) void feedPools(c.env, out.file, out.rows, c.get("uid"));
...
async function purgeFile(env: Env, file: SheetFile, uid: string) { let wiped: { rows: Row[] }; try { wiped = await rpc(env.FILES, file.id, "wipe", { uid }) as { rows: Row[] }; } catch (e: any) { console.error("purge wipe failed", file.id, e?.message ?? e); throw new Error("could not wipe file"); } await rpc(env.INDEX, "global", "purge", { id: file.id }); if (file.password) await removePoolRows(env, file.password, wiped.rows as Row[], uid, file); }
