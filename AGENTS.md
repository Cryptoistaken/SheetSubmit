...
components/layout/Sidebar.tsx         # collapsible admin-only rail, same on desktop + phones (default 240px, icons 64px, ss_sidebar_mode expanded|icons|tight|mini|hidden + ss_sidebar_width; trigger toggles expanded↔icons, hidden only via X button with floating expand back; hover auto-expands only with (hover:hover) mice, tap trigger on touch; edge-drag stops 64/48/36 via 24px touch zone + hover-only grip (release <42 mini, <56 tight, <160 icons), <6px move ignored as tap; mini footer compacts (p-1, size-6 trigger, w-3 handle); separator focusable with arrow keys; logo block h-12 matches topbar): logo header, home-section links + Admin group, footer with files-tab ViewSwitch (expanded only) + panel trigger + hide (X) button (expanded only, jumps to hidden) (hover dead-zone: footer hover never expands/collapses, click toggles); admins hide the tab bar (#homeTabBar.admin-tabs) and get the breadcrumb header (no dup logo), regulars keep Topbar + tabs + floating toggle everywhere; profile/balance/theme live in the topbar, never the sidebar; sheet pages never mount it
