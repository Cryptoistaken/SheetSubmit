...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "batchArchive": await db.begin(async (tx: any) => { for (const f of a.files as SheetFile[]) await tx`UPDATE file_index SET archived=false,data=${j(f)},updated_at=now() WHERE file_id=${f.id}`; }); return { ok: true };
...
async function poolOp(password: string, op: string, a: any) {
  ...
  if (op === "removeAvailable") return db.begin(async (tx: any) => { for (const k of a.keys || []) await tx`DELETE FROM pool_rows WHERE password=${password} AND pool_id=${a.pool} AND row_key=${k} AND state='available' AND src_file_id=${a.srcFileId ?? ""}`; return { ok: true }; });
