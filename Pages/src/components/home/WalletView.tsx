...
function UserWallet() {
  ...
  return <div className="flex flex-col gap-6"><WalletBalance balance={wallet.balance}/><form onSubmit={submit} className="rounded-xl border bg-card p-6"><h2 className="text-sm font-semibold">Request a withdrawal</h2><p className="mt-1 text-sm text-muted-foreground">Requests are reviewed before payment is sent.</p><div className="mt-4 grid gap-4 sm:grid-cols-3"><label className="flex flex-col gap-2 text-sm font-medium">Amount<input className="h-10 rounded-md border bg-background px-3 font-normal" type="number" min="0.01" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0.00" /></label><label className="flex flex-col gap-2 text-sm font-medium">Method<select className="h-10 rounded-md border bg-background px-3 font-normal" value={method} onChange={(e) => setMethod(e.target.value)}><option>bKash</option><option>Nagad</option><option>Rocket</option><option>Crypto</option></select></label><label className="flex flex-col gap-2 text-sm font-medium">Payout account<input className="h-10 rounded-md border bg-background px-3 font-normal" value={account} onChange={(e) => setAccount(e.target.value)} placeholder="Account number or wallet address" /></label></div><button className="mt-4 h-10 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50" disabled={sending} type="submit">{sending ? "Sending…" : "Request withdrawal"}</button></form><WithdrawalHistory items={wallet.withdrawals}/></div>;
}
