...
app.route("/api", wa);
app.route("/", bot);
app.get("/api/auth/me", async (c) => { const token = c.req.header("Cookie")?.match(/(?:^|;\s*)ss_session=([^;]+)/)?.[1]; if (!token) return c.json({ error: "not_authenticated" }, 401); const session = await verifySession(token, c.env.SESSION_SECRET); if (!session) return c.json({ error: "session_expired" }, 401); const user = await rpc(c.env.INDEX, "global", "user", { id: session.uid }); if (!user) return c.json(null); return c.json({ ...user, photoUrl: c.env.WORKER_URL ? `https://${c.env.WORKER_URL}/api/auth/photo/${session.uid}` : null, isAdmin: isAdmin(c.env, session.uid) }); });
app.get("/api/auth/photo/:userId", async (c) => { const uid = c.req.param("userId"); if (!/^\d{3,20}$/.test(uid) || !c.env.TG_BOT_TOKEN) return c.text("not found", 404); const cached: any = await rpc(c.env.INDEX, "global", "metaGet", { k: `photo:${uid}` }).catch(() => null); let path: string | null = cached?.path && cached?.ts && Date.now() - cached.ts < 86400_000 ? cached.path : null; if (!path) { try { const tg = (m: string, b: unknown) => fetch(`https://api.telegram.org/bot${c.env.TG_BOT_TOKEN}/${m}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json() as any); const p = await tg("getUserProfilePhotos", { user_id: Number(uid), limit: 1 }); const fid = p?.result?.photos?.[0]?.slice(-1)?.[0]?.file_id; if (fid) { const g = await tg("getFile", { file_id: fid }); path = g?.result?.file_path ?? null; if (path) await rpc(c.env.INDEX, "global", "metaSet", { k: `photo:${uid}`, v: { path, ts: Date.now() } }).catch(() => {}); } } catch {} } if (!path) return c.text("not found", 404); return c.redirect(`https://api.telegram.org/file/bot${c.env.TG_BOT_TOKEN}/${path}`, 302); });
