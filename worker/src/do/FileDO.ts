...
export class FileDO {
  ...
  async fetch(req: Request): Promise<Response> { const { op, args = {} } = await req.json() as { op: string; args?: any }; const s = this.state.storage.sql; switch (op) {
     case "init": s.exec("BEGIN"); try { s.exec("DELETE FROM meta; DELETE FROM rows; DELETE FROM logs"); s.exec("INSERT INTO meta(k,v) VALUES('file',?)", JSON.stringify(args.file)); s.exec("INSERT INTO meta(k,v) VALUES('seq','0')"); (args.rows || []).forEach((r: Row, i: number) => s.exec("INSERT INTO rows(idx,data) VALUES(?,?)", i, JSON.stringify(r))); s.exec("COMMIT"); } catch (e) { s.exec("ROLLBACK"); throw e; } return Response.json({ ok: true });
