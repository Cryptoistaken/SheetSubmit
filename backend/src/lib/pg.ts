...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "metaGetMany": { const rows: any[] = await db`SELECT k,v FROM meta WHERE k=ANY(${db.array((a.keys || []).slice(0, 1000))})`; return Object.fromEntries(rows.map((r) => [r.k, json(r.v)])); }
