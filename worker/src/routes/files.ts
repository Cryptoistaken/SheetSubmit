...
export const ldCounts = (rows: Row[]) => { let live = 0, dead = 0; for (const r of rows) { const s = String(r.status || "").toLowerCase(); if (s === "good") live++; else if (s === "bad") dead++; } return { liveCount: live, deadCount: dead }; };
