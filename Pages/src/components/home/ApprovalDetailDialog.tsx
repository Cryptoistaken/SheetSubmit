    ...
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
       <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>{hold.poolId} · {qty} qty · {hold.mode ?? "—"} · {st}</DialogDescription>
        </DialogHeader>
        {isApproved ? <div className="py-2"><InkStamp label="APPROVED" /></div> : null}
        <div className="flex flex-col gap-2 text-sm">
          <div><span className="font-semibold">Status:</span> {st}</div>
          ...
          {users.length ? (
            <div className="flex flex-col gap-2">
              {users.map((uid) => {
                ...
                return <div key={uid} className="rounded-md border bg-muted p-3"><div className="font-semibold">User #{uid.slice(-8)} <span className="text-xs font-normal text-muted-foreground">{files.reduce((sum, file) => sum + file.count, 0)} rows · {files.length} files</span></div><div className="mt-1 flex flex-col gap-1 text-xs text-muted-foreground">{files.map((file) => <div key={`${file.srcFileId}-${file.count}`}>File #{file.srcFileId?.slice(-8) ?? "unknown"} · {file.count} rows</div>)}</div></div>
              })}
            </div>
