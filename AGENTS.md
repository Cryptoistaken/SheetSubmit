...
```
  index.ts              # app setup, routes, API_VERSION (currently 2.0.20; bump on any route change, surfaced by /api/health), typed JSON errors (known client failures → 4xx with message, unknown masked as 500 + logged with method+path),
