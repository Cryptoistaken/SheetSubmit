...
pages/BubbleDesignPage.tsx   # /admin renders via HomePage + components/home/AdminView.tsx (no AdminPage file)
 components/layout/Topbar.tsx          # connection card + shadcn profile dropdown + animated theme toggle + admin-only hamburger (mobile drawer trigger, lg:hidden) (mobile header everywhere; desktop home: route breadcrumb left + utility cluster right via .home-topbar CSS, full header on sheet pages)
components/layout/Sidebar.tsx         # collapsible nav (240px ↔ 64px icons, ss_sidebar_collapsed, hover auto-expands on desktop; slide-over drawer on phones, admin-only via Layout mobileNavOpen + backdrop + Escape): logo header, home-section links + Admin group, footer with panel trigger only (hover dead-zone: footer hover never expands/collapses, click toggles); admins hide the phone tab bar (#homeTabBar.admin-tabs), regulars keep Topbar + tabs everywhere; profile/balance/theme live in the topbar, never the sidebar; sheet pages never mount it
