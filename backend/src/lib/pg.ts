...
async function poolOp(password: string, op: string, a: any) {
  ...
  if (op === "removeFileRows") { const fid = String(a.srcFileId || ""); if (!fid) return { removed: 0 }; const r: any = await db`DELETE FROM pool_rows WHERE password=${password} AND src_file_id=${fid} AND state IN ('available','held')`; return { removed: Number(r.count || 0) }; }
