...
wa.post("/fb/page-simple", async (c) => { if (!(await rateLimit(`rl:fb:simple:${c.get("uid")}`, 30, 60))) return c.json({ eligible: false, banReason: null, linkedNumber: null, pageName: null, error: "Rate limited", code: "rate_limited" }); const { cookie } = await c.req.json<{ cookie?: string }>().catch(() => ({}) as { cookie?: string }); if (!cookie) return c.json({ error: "Cookie required", code: "invalid_cookie" }, 400); if (cookie.length > 50000) return c.json({ error: "Cookie too large", code: "invalid_cookie" }, 400); const fail = (error: string | null, code = "upstream_error") => c.json({ eligible: false, banReason: null, linkedNumber: null, pageName: null, error, code });
  try {
    ...
    const pageName = pages[0]?.name ?? null; const eligible = pages.length > 0 && !!pageName;
    if (cuser) { if (eligible) await checkCacheSet(c.env, c.get("uid"), cuser, { status: "eligible", banReason: null, pageName, linkedNumber, error: null, ts: Date.now() }); else await checkCacheDel(c.env, c.get("uid"), cuser); }
    return c.json({ eligible, banReason: null, linkedNumber, pageName, error: null });
...
wa.post("/fb/page-advanced", async (c) => { if (!(await rateLimit(`rl:fb:advanced:${c.get("uid")}`, 30, 60))) return c.json({ eligible: false, banReason: null, linkedNumber: null, pageName: null, error: "Rate limited", code: "rate_limited" }); const { cookie } = await c.req.json<{ cookie?: string }>().catch(() => ({}) as { cookie?: string }); if (!cookie) return c.json({ error: "Cookie required", code: "invalid_cookie" }, 400); if (cookie.length > 50000) return c.json({ error: "Cookie too large", code: "invalid_cookie" }, 400); const fail = (error: string, code = "upstream_error") => c.json({ eligible: false, banReason: null, linkedNumber: null, pageName: null, error, code });
  try {
    ...
    const result = { eligible: elig?.is_eligible === true && !!pageName, banReason: elig?.ban_reason || null, linkedNumber: elig?.page_whatsapp_number || null, pageName, error: null };
    if (cuser) { if (result.eligible) await checkCacheSet(c.env, c.get("uid"), cuser, { status: "eligible", banReason: result.banReason, pageName, linkedNumber: result.linkedNumber, error: null, ts: Date.now() }); else if (result.error === null) await checkCacheDel(c.env, c.get("uid"), cuser); }
