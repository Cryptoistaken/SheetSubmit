...
pages/BubbleDesignPage.tsx   # /admin renders via HomePage + components/home/AdminView.tsx (no AdminPage file)
 components/layout/Topbar.tsx          # connection card + shadcn profile dropdown + animated theme toggle (mobile header everywhere; desktop header on sheet pages only)
components/layout/Sidebar.tsx         # desktop-only (lg:+) collapsible nav (240px ↔ 64px icons, ss_sidebar_collapsed, hover auto-expands): logo header, account card top (avatar + name + logout + USDC/BDT icon balance toggle), home-section links + Admin group, footer with panel trigger + theme toggle; sheet pages never mount it
