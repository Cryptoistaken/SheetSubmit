...
components/layout/Sidebar.tsx         # collapsible admin-only rail, same on desktop + phones (default 240px, icons 64px, ss_sidebar_collapsed; hover auto-expands on desktop, tap trigger on touch; edge-drag resize 64–320px in ss_sidebar_width, release <120px snaps collapsed; separator focusable with arrow keys; logo block h-12 matches topbar): logo header, home-section links + Admin group, footer with files-tab ViewSwitch (expanded only) + panel trigger (hover dead-zone: footer hover never expands/collapses, click toggles); admins hide the tab bar (#homeTabBar.admin-tabs), regulars keep Topbar + tabs + floating toggle everywhere; profile/balance/theme live in the topbar, never the sidebar; sheet pages never mount it
