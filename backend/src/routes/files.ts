...
archive.get("/:id/rows", async (c) => { const file = await ownedArchived(c, c.req.param("id")); if (!file) return c.json({ error: "not found" }, 404); return c.json(await rpc(c.env.FILES, file.id, "rows").catch(() => [] as Row[])); });
archive.get("/:id/full", async (c) => { const file = await ownedArchived(c, c.req.param("id")); if (!file) return c.json({ error: "not found" }, 404); const full = await rpc(c.env.FILES, file.id, "full").catch(() => ({ rows: [], seq: 0 })) as { rows: Row[]; seq: number }; return c.json({ file, rows: full.rows ?? [], seq: full.seq ?? 0 }); });
