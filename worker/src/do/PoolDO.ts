...
function classifyRow(r: Row, preset?: string | null): Pool | null { const c = String(r.cookies || ""); if (!/c_user=\d+/.test(c) || !r.uid || ["bad", "dead"].includes(String(r.status || "").toLowerCase())) return null; const has2 = hasReal2FA(r); if (preset === "page") return has2 ? "page" : "cookies_only"; if (preset === "combo" || preset === "2fa") return has2 ? "cookies_2fa" : "cookies_only"; if (preset === "cookie") return "cookies_only"; const two = has2; if (String(r.wa_status || r.waStatus || "") === "eligible" && two) return "page"; return two ? "cookies_2fa" : "cookies_only"; }
