...
export class FileDO {
  constructor(private readonly state: DurableObjectState) { state.blockConcurrencyWhile(async () => { const s = state.storage.sql; s.exec("CREATE TABLE IF NOT EXISTS meta (k TEXT PRIMARY KEY,v TEXT NOT NULL)"); s.exec("CREATE TABLE IF NOT EXISTS rows (idx INTEGER PRIMARY KEY,data TEXT NOT NULL)"); s.exec("INSERT OR IGNORE INTO meta(k,v) VALUES('seq','0')"); }); }
  ...
  async fetch(req: Request): Promise<Response> { const { op, args = {} } = await req.json() as { op: string; args?: any }; const s = this.state.storage.sql; switch (op) {
    case "init": s.exec("DELETE FROM meta; DELETE FROM rows"); s.exec("INSERT INTO meta(k,v) VALUES('file',?)", JSON.stringify(args.file)); s.exec("INSERT INTO meta(k,v) VALUES('seq','0')"); (args.rows || []).forEach((r: Row, i: number) => s.exec("INSERT INTO rows(idx,data) VALUES(?,?)", i, JSON.stringify(r))); return Response.json({ ok: true });
