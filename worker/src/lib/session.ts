...
export async function signSession(uid: string, secret: string | undefined) { const body = b64(JSON.stringify({ uid, exp: Date.now() + 30 * 86400000 })); const sig = b64(await crypto.subtle.sign("HMAC", await key(secret || "", ["sign"]), enc.encode(body))); return `${body}.${sig}`; }
export async function verifySession(token: string, secret: string | undefined) { const [body, sig] = token.split("."); if (!body || !sig) return null; const valid = await crypto.subtle.verify("HMAC", await key(secret || "", ["verify"]), Uint8Array.from(unb64(sig), (c) => c.charCodeAt(0)), enc.encode(body)); if (!valid) return null; try { const data = JSON.parse(unb64(body)); return data.exp > Date.now() ? { uid: String(data.uid) } : null; } catch { return null; } }
