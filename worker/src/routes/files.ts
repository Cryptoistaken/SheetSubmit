...
async function feedPools(env: Env, file: SheetFile, rows: Row[], uid: string) { if (file.poolEnabled === false || !file.password) return; await rpc(env.POOLS, file.password, "add", { rows, uid, srcUid: uid, srcFileId: file.id }).catch(() => {}); }
