...
// delete-time pool wipe for ONE file: available + held rows go, claimed (sold)
// rows stay — money moved on those and the buyer owns the snapshot.
...
const claimedBlock = (n: number) => ({ error: `${n} sold row(s) of this file are claimed and cannot be permanently deleted — the buyer owns them` });
export { filePoolCounts, claimedBlock };
...
archive.post("/batch-restore", async (c) => { const body = await c.req.json<{ ids?: unknown }>().catch(() => ({ ids: undefined })); const ids = [...new Set(Array.isArray(body.ids) ? body.ids.filter((id): id is string => typeof id === "string") : [])]; if (!ids.length) return c.json({ error: "no ids" }, 400); if (ids.length > 40) return c.json({ error: "too many ids" }, 400); const archived = await rpc(c.env.INDEX, "global", "files", { uid: c.get("uid"), archived: 1 }) as SheetFile[]; const files = archived.filter((f) => ids.includes(f.id)); files.forEach((f) => { delete (f as any).deletedAt; f.lastAction = "restored"; }); if (files.length) await rpc(c.env.INDEX, "global", "batchArchive", { files }); for (const f of files) { const rows = await rpc(c.env.FILES, f.id, "rows").catch(() => [] as Row[]) as Row[]; if (rows.length) void feedPools(c.env, f, rows, c.get("uid")); } return c.json({ restored: files.length }); });
  archive.delete("/:id", async (c) => { const file = await ownedArchived(c, c.req.param("id")); if (!file) return c.json({ error: "not found" }, 404); let counts = { held: 0, claimed: 0 }; try { counts = await filePoolCounts(c, file); } catch { return c.json({ error: "could not verify pool state, refusing to delete" }, 503); } if (counts.claimed) return c.json(claimedBlock(counts.claimed), 409); try { await purgeFile(c.env, file, c.get("uid")); } catch { return c.json({ error: "could not delete file" }, 503); } return c.json({ ok: true }); });
   archive.post("/batch-delete", async (c) => { const body = await c.req.json<{ ids?: unknown }>().catch(() => ({ ids: undefined })); const ids = [...new Set(Array.isArray(body.ids) ? body.ids.filter((id): id is string => typeof id === "string") : [])]; if (!ids.length) return c.json({ error: "no ids" }, 400); if (ids.length > 20) return c.json({ error: "too many ids" }, 400); const archived = await rpc(c.env.INDEX, "global", "files", { uid: c.get("uid"), archived: 1 }) as SheetFile[]; const owned = archived.filter((f) => ids.includes(f.id)); if (!owned.length) return c.json({ deleted: 0 }); for (const f of owned) { let counts = { held: 0, claimed: 0 }; try { counts = await filePoolCounts(c, f); } catch { return c.json({ error: `${f.name}: could not verify pool state` }, 503); } if (counts.claimed) return c.json({ error: `${f.name}: ${claimedBlock(counts.claimed).error}` }, 409); } for (const f of owned) { try { await rpc(c.env.FILES, f.id, "wipe", { uid: c.get("uid") }); } catch { return c.json({ error: `could not wipe ${f.name}` }, 503); } } await rpc(c.env.INDEX, "global", "batchPurge", { ids: owned.map((f) => f.id) });   await Promise.all(owned.map((f) => f.password ? removeFilePoolRows(c.env, f.password, f.id) : Promise.resolve())); return c.json({ deleted: owned.length }); });
