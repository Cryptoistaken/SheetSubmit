...
async function poolOp(password: string, op: string, a: any) {
  ...
  if (op === "verifiedCounts") { const all: any[] = await db`SELECT data FROM pool_rows WHERE password=${password} AND pool_id=${p} AND state='available' LIMIT 5000`; const total: any = (await db`SELECT COUNT(*) n FROM pool_rows WHERE password=${password} AND pool_id=${p} AND state='available'`)[0]; const verified = all.filter((r) => eligible(json(r.data))).length; return { pool: p, verified, unverified: all.length - verified, totalAvailable: Number(total.n), ...(p === "page" ? { totalCookies2faAvailable: Number(total.n), unverifiedScanned: all.length } : {}), truncated: Number(total.n) > 5000, scanCap: 5000 }; }
  ...
  if (op === "markDead") { const keys = [...new Set((Array.isArray(a.dead) ? a.dead : []).map(String))].slice(0, 500); if (!keys.length) return { dead: 0 }; const r: any = await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND row_key IN ${db(keys)}`; return { dead: Number(r.count || 0) }; }
