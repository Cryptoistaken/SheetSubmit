...
async function fileOp(id: string, op: string, a: any) {
  ...
  if (op === "wipe") return db.transaction(async (tx: any) => { const rows = (await tx`SELECT data FROM file_rows WHERE file_id=${id}`).map((r: any) => json(r.data)); await tx`DELETE FROM file_rows WHERE file_id=${id}`; await tx`DELETE FROM file_logs WHERE file_id=${id}`; await tx`DELETE FROM file_meta WHERE file_id=${id}`; return { ok: true, rows }; });
...
async function poolOp(password: string, op: string, a: any) {
  const p = String(a.pool || ""); if (["priceGet", "priceSet", "summary", "detail", "counts", "claim", "hold", "verifiedCounts", "userFiles"].includes(op) && !pools.includes(p as Pool)) throw new Error("invalid pool");
  ...
  if (op === "removeAvailable") return db.transaction(async (tx: any) => { for (const k of a.keys || []) await tx`DELETE FROM pool_rows WHERE password=${password} AND pool_id=${a.pool} AND row_key=${k} AND state='available' AND src_file_id=${a.srcFileId ?? ""}`; return { ok: true }; });
  ...
  if (op === "markDead") { const keys = [...new Set((Array.isArray(a.dead) ? a.dead : []).map(String))].slice(0, 500); if (!keys.length) return { dead: 0 }; const pool = typeof a.pool === "string" && a.pool ? String(a.pool) : null; const r: any = password === "global" ? (pool ? await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND pool_id=${pool} AND row_key IN ${db(keys)}` : await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND row_key IN ${db(keys)}`) : (pool ? await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND password=${password} AND pool_id=${pool} AND row_key IN ${db(keys)}` : await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND password=${password} AND row_key IN ${db(keys)}`); return { dead: Number(r.count || 0) }; }
...
async function selectRows(tx: any, password: string, pool: string, a: any, limit: number) {
  ...
  const rows: any[] = u.length && f.length
    ? await tx`SELECT row_key,data FROM pool_rows WHERE password=${password} AND pool_id=${pool} AND state='available' AND src_uid IN ${db(u)} AND src_file_id IN ${db(f)} ORDER BY inserted_at,row_key LIMIT ${cap} FOR UPDATE SKIP LOCKED`
    : u.length
      ? await tx`SELECT row_key,data FROM pool_rows WHERE password=${password} AND pool_id=${pool} AND state='available' AND src_uid IN ${db(u)} ORDER BY inserted_at,row_key LIMIT ${cap} FOR UPDATE SKIP LOCKED`
      : f.length
        ? await tx`SELECT row_key,data FROM pool_rows WHERE password=${password} AND pool_id=${pool} AND state='available' AND src_file_id IN ${db(f)} ORDER BY inserted_at,row_key LIMIT ${cap} FOR UPDATE SKIP LOCKED`
...
async function allocate(password: string, op: string, pool: string, a: any) {
  ...
  return db.transaction(async (tx: any) => { const rows = await selectRows(tx, password, pool, a, want); if (!rows.length) return op === "hold" ? { claimed: 0, held: 0, count: 0, rows: [], holdId: null, downloadId: null, filename: null, status: "HOLD", unitPrice: price(pool), total: 0, mode: a.mode || "fifo" } : { claimed: 0, rows: [], downloadId: null, filename: a.filename || null, status: null, unitPrice: price(pool), total: 0, mode: "fifo" };
    const id = String(a.downloadId || a.holdId || crypto.randomUUID().replaceAll("-", "").slice(0, 12)), now = Date.now(), state = op === "hold" ? "held" : "claimed", status = op === "hold" ? "HOLD" : "CLAIMED"; const unit = Number(((await tx`SELECT price FROM pool_settings WHERE password=${password} AND pool_id=${pool}`)[0]?.price ?? price(pool))); const data = rows.map((r: any) => r.data), keys = rows.map((r: any) => r.row_key); await tx`UPDATE pool_rows SET state=${state},hold_id=${op === "hold" ? id : null},claimed_by=${a.uid},claimed_at=${now} WHERE password=${password} AND pool_id=${pool} AND state='available' AND row_key IN ${db(keys)}`;   const filename = String(a.filename || `${pool}_${id.slice(-4)}.xlsx`); await tx`INSERT INTO downloads(id,password,pool_id,claimed_by,claimed,filename,keys,rows,reverted,ts,status,unit_price,total,mode,src_uids,src_file_ids,selection) VALUES(${id},${password},${pool},${a.uid},${rows.length},${filename},${j(keys)},${j(data)},false,${now},${status},${unit},${+(unit * rows.length).toFixed(2)},${a.mode || "fifo"},${j(a.srcUids || (a.srcUid ? [String(a.srcUid)] : null))},${j(a.srcFileIds || (a.srcFileId ? [String(a.srcFileId)] : null))},${j({ verifiedOnly: !!a.verifiedOnly, unverifiedOnly: !!a.unverifiedOnly })}) ON CONFLICT(id) DO NOTHING`; return op === "hold" ? { claimed: rows.length, held: rows.length, count: rows.length, rows: data, holdId: id, downloadId: id, filename, status, unitPrice: unit, total: +(unit * rows.length).toFixed(2), mode: a.mode || "fifo", srcUids: a.srcUids || null, srcFileIds: a.srcFileIds || null } : { claimed: rows.length, rows: data, downloadId: id, filename, status, unitPrice: unit, total: +(unit * rows.length).toFixed(2), mode: "fifo" }; });
}
