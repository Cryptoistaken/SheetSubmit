...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "batchArchive": await db.transaction(async (tx: any) => { for (const f of a.files as SheetFile[]) await tx`UPDATE file_index SET archived=false,data=${tx.json(f)},updated_at=now() WHERE file_id=${f.id}`; }); return { ok: true };
