...
src/routes/pools.ts       # admin pool, hold, download, ledger and pricing routes
                      ...
                      #   GET /holds (status filter), POST /holds/:id/approve + /reject (REVERT WINDOW: first action starts 5min, exactly one flip allowed, then final — pg.ts throws "decision is final — revert window closed"; NO wallet ops at action time),
