...
files.get("/", async (c) => c.json(await rpc(c.env.INDEX, "global", "files", { uid: c.get("uid") })));
 files.post("/", async (c) => {
  const body = await c.req.json<Partial<SheetFile> & { rows?: Row[]; dataCount?: number }>(); const rows = Array.isArray(body.rows) ? body.rows : []; if (rows.length > 50000) return c.json({ error: "too many rows (max 50000)" }, 400); const name = String(body.name || "Untitled").slice(0, 128); if (Array.isArray(body.columns) && body.columns.length > 100) return c.json({ error: "too many columns" }, 400); const rawPreset = (body as any).preset ?? (body as any).poolKind; let preset = normalizePreset(rawPreset); if (!preset) { const tmp: SheetFile = { id: "", name, type: "fb_cookie", columns: Array.isArray(body.columns) ? body.columns as any : undefined } as SheetFile; preset = resolvePreset(tmp) ?? undefined; } const file: SheetFile = { id: fileId(), name, type: body.type === "fb_cookie" ? "fb_cookie" : "fb_cookie", ...(preset ? { preset, poolKind: preset } : {}), password: String(body.password || "dgddigital").slice(0, 64), poolEnabled: body.poolEnabled !== false, ...(Array.isArray(body.columns) ? { columns: body.columns } : {}), createdAt: Date.now(), updatedAt: Date.now(), rowCount: rows.length, dataCount: body.dataCount ?? 0, lastAction: "created" }; Object.assign(file, ldCounts(rows)); await rpc(c.env.INDEX, "global", "register", { uid: c.get("uid"), file }); await rpc(c.env.FILES, file.id, "init", { file, rows }); if (rows.length) void feedPools(c.env, file, rows, c.get("uid")); return c.json(file); });
files.put("/:id", async (c) => { const file = await owned(c, c.req.param("id")); if (!file) return c.json({ error: "file not found" }, 404); const body = await c.req.json<Record<string, unknown>>(); const prevPassword = (file as any).password; for (const k of ["name", "type", "columns", "password", "poolEnabled", "preset", "poolKind"]) if (k in body) (file as any)[k] = body[k]; if (typeof (file as any).name === "string") (file as any).name = (file as any).name.slice(0, 128); if (typeof (file as any).password === "string") (file as any).password = (file as any).password.slice(0, 64); if (Array.isArray((file as any).columns) && (file as any).columns.length > 100) return c.json({ error: "too many columns" }, 400); if ("poolKind" in body && !("preset" in body)) (file as any).preset = (file as any).poolKind; const np = normalizePreset((file as any).preset ?? (file as any).poolKind); if (np) { (file as any).preset = np; (file as any).poolKind = np; } if (prevPassword && (file as any).password && prevPassword !== (file as any).password) return c.json({ error: "password change not allowed — create a new file instead (pool rows are keyed by password)" }, 400); file.updatedAt = Date.now(); file.lastAction = "renamed"; await rpc(c.env.FILES, file.id, "save", { file }); await rpc(c.env.INDEX, "global", "register", { uid: c.get("uid"), file }); if ((body as any).poolEnabled === false && (file as any).password) { try { const rows = await rpc(c.env.FILES, file.id, "rows").catch(() => [] as Row[]) as Row[]; await removePoolRows(c.env, (file as any).password, rows, c.get("uid"), file); } catch (e: any) { console.error("pool disable cleanup failed", file.id, e?.message ?? e); } } return c.json(file); });
...
files.put("/:id/append", async (c) => {
  ...
  void feedPools(c.env, updated, saved.rows ?? [], c.get("uid"));
...
async function purgeFile(env: Env, file: SheetFile, uid: string) { let wiped: { rows: Row[] }; try { wiped = await rpc(env.FILES, file.id, "wipe") as { rows: Row[] }; } catch (e: any) { console.error("purge wipe failed", file.id, e?.message ?? e); throw new Error("could not wipe file"); } await rpc(env.INDEX, "global", "purge", { id: file.id }); if (file.password) await removePoolRows(env, file.password, wiped.rows as Row[], uid, file); }
...
archive.post("/:id/restore", async (c) => { const file = await ownedArchived(c, c.req.param("id")); if (!file) return c.json({ error: "not found" }, 404); delete (file as any).deletedAt; file.lastAction = "restored"; await rpc(c.env.INDEX, "global", "archive", { id: file.id, archived: false, file }); const rows = await rpc(c.env.FILES, file.id, "rows").catch(() => [] as Row[]) as Row[]; if (rows.length) void feedPools(c.env, file, rows, c.get("uid")); return c.json({ ok: true }); });
archive.post("/batch-restore", async (c) => { const body = await c.req.json<{ ids?: unknown }>().catch(() => ({ ids: undefined })); const ids = [...new Set(Array.isArray(body.ids) ? body.ids.filter((id): id is string => typeof id === "string") : [])]; if (!ids.length) return c.json({ error: "no ids" }, 400); if (ids.length > 40) return c.json({ error: "too many ids" }, 400); const archived = await rpc(c.env.INDEX, "global", "files", { uid: c.get("uid"), archived: 1 }) as SheetFile[]; const files = archived.filter((f) => ids.includes(f.id)); files.forEach((f) => { delete (f as any).deletedAt; f.lastAction = "restored"; }); if (files.length) await rpc(c.env.INDEX, "global", "batchArchive", { files }); for (const f of files) { const rows = await rpc(c.env.FILES, f.id, "rows").catch(() => [] as Row[]) as Row[]; if (rows.length) void feedPools(c.env, f, rows, c.get("uid")); } return c.json({ restored: files.length }); });
  archive.delete("/:id", async (c) => { const file = await ownedArchived(c, c.req.param("id")); if (!file) return c.json({ error: "not found" }, 404); let wipedRows: Row[]; try { wipedRows = await rpc(c.env.FILES, file.id, "rows") as Row[]; } catch { return c.json({ error: "could not read file rows, refusing to delete" }, 503); } let held = 0; try { held = await heldInRows(c, file, wipedRows); } catch { return c.json({ error: "could not verify hold state, refusing to delete" }, 503); } if (held) return c.json(heldBlock(held), 409); try { await purgeFile(c.env, file, c.get("uid")); } catch { return c.json({ error: "could not delete file" }, 503); } return c.json({ ok: true }); });
   archive.post("/batch-delete", async (c) => { const body = await c.req.json<{ ids?: unknown }>().catch(() => ({ ids: undefined })); const ids = [...new Set(Array.isArray(body.ids) ? body.ids.filter((id): id is string => typeof id === "string") : [])]; if (!ids.length) return c.json({ error: "no ids" }, 400); if (ids.length > 20) return c.json({ error: "too many ids" }, 400); const archived = await rpc(c.env.INDEX, "global", "files", { uid: c.get("uid"), archived: 1 }) as SheetFile[]; const owned = archived.filter((f) => ids.includes(f.id)); if (!owned.length) return c.json({ deleted: 0 }); for (const f of owned) { let rows: Row[]; try { rows = await rpc(c.env.FILES, f.id, "rows") as Row[]; } catch { return c.json({ error: `${f.name}: could not read rows, refusing to delete` }, 503); } let held = 0; try { held = await heldInRows(c, f, rows); } catch { return c.json({ error: `${f.name}: could not verify hold state` }, 503); } if (held) return c.json({ error: `${f.name}: ${heldBlock(held).error}` }, 409); } const wiped: { rows: Row[] }[] = []; for (const f of owned) { try { wiped.push(await rpc(c.env.FILES, f.id, "wipe") as { rows: Row[] }); } catch { return c.json({ error: `could not wipe ${f.name}` }, 503); } } await rpc(c.env.INDEX, "global", "batchPurge", { ids: owned.map((f) => f.id) });   await Promise.all(owned.map((f, i) => f.password ? removePoolRows(c.env, f.password, wiped[i].rows, c.get("uid"), f) : Promise.resolve())); return c.json({ deleted: owned.length }); });
