...
export function ThemeTogglerButton({ theme, onToggle }: { theme: "light" | "dark"; onToggle: () => void }) {
  ...
  return (
    <Button type="button" variant="ghost" size="icon" className="sweepable" aria-label={`Switch to ${dark ? "light" : "dark"} theme`} title={`Switch to ${dark ? "light" : "dark"} theme`} onClick={(e) => { sweepClick(e); onToggle(); }}>
