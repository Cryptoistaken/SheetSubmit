...
pages/BubbleDesignPage.tsx   # /admin renders via HomePage + components/home/AdminView.tsx (no AdminPage file)
 components/layout/Topbar.tsx          # connection card + shadcn profile dropdown + animated theme toggle (mobile header everywhere; desktop home: route breadcrumb left + utility cluster right via .home-topbar CSS, full header on sheet pages)
components/layout/Sidebar.tsx         # desktop-only (lg:+) collapsible nav (240px ↔ 64px icons, ss_sidebar_collapsed, hover auto-expands): logo header, home-section links + Admin group, footer with panel trigger only (hover dead-zone: footer hover never expands/collapses, click toggles); profile/balance/theme live in the topbar, never the sidebar; sheet pages never mount it
