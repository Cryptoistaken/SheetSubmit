...
app.get("/api/auth/photo/:userId", requireAuth, async (c) => { const uid = c.req.param("userId") || ""; const botToken = c.env.TG_BOT_TOKEN; if (!/^\d{3,20}$/.test(uid) || !botToken) return c.text("not found", 404); const cached: any = await rpc(c.env.INDEX, "global", "metaGet", { k: `photo:${uid}` }).catch(() => null); let path: string | null = cached?.path ?? null; if (!path) { try { const tg = (m: string, b: unknown) => fetch(`https://api.telegram.org/bot${botToken}/${m}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(b) }).then((r) => r.json() as any); const p = await tg("getUserProfilePhotos", { user_id: Number(uid), limit: 1 }); const fid = p?.result?.photos?.[0]?.slice(-1)?.[0]?.file_id; if (fid) { const g = await tg("getFile", { file_id: fid }); path = g?.result?.file_path ?? null; if (path) await rpc(c.env.INDEX, "global", "metaSet", { k: `photo:${uid}`, v: { path, ts: Date.now() } }).catch(() => {}); } } catch {} } if (!path) return c.text("not found", 404); return c.redirect(`https://api.telegram.org/file/bot${botToken}/${path}`, 302); });
