...
async function poolOp(password: string, op: string, a: any) {
  ...
  if (op === "markDead") { const keys = [...new Set((Array.isArray(a.dead) ? a.dead : []).map(String))].slice(0, 500); if (!keys.length) return { dead: 0 }; const r: any = await db`UPDATE pool_rows SET state='dead' WHERE state='available' AND row_key=ANY(${db.array(keys)}::text[])`; return { dead: Number(r.count || 0) }; }
