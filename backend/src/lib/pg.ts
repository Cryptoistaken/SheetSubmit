...
const eligible = (r: any) => String(r?.check_status ?? r?.wa_status ?? r?.waStatus ?? "").toLowerCase() === "eligible";
...
function counts(rows: Row[]) { let live = 0, dead = 0, page = 0, dup = 0; const keys = new Map<string, number>(); for (const r of rows) { const s = String(r.status || "").toLowerCase(); if (s === "good") live++; else if (s === "bad") dead++; if (String((r as any).check_status ?? (r as any).wa_status ?? "").toLowerCase() === "eligible") page++; const k = key(r); if (k) keys.set(k, (keys.get(k) || 0) + 1); } keys.forEach((n) => { if (n > 1) dup += n; }); return { liveCount: live, deadCount: dead, pageCount: page, dupCount: dup }; }
...
async function indexOp(op: string, a: any) {
  switch (op) {
    ...
    case "crossDups": {
      // R2: one query replaces the per-file dupKeys fan-out — per-type file
      // counts from file_index, key groups over file_rows JOIN file_index
      // (same owner + non-archived filters, row_key IS NOT NULL), dup = key
      // seen >1 times within a type that has >1 file. COALESCE on type keeps
      // legacy typeless files grouped exactly like the old JS byType[f.type].
      const uid = String(a.uid || ""), type = a.type == null ? null : String(a.type);
      const rows: any[] = await db`
        WITH type_counts AS (
          SELECT COALESCE(data->>'type', '') AS type, COUNT(*) AS n
          FROM file_index WHERE owner_id=${uid} AND archived=false GROUP BY COALESCE(data->>'type', '')
        ),
        groups AS (
          SELECT COALESCE(f.data->>'type', '') AS type, r.row_key AS k,
            jsonb_agg(jsonb_build_object('fileId', r.file_id, 'fileName', f.data->>'name', 'rowIdx', r.idx) ORDER BY r.file_id, r.idx) AS entries,
            COUNT(*) AS n
          FROM file_rows r JOIN file_index f ON f.file_id=r.file_id
          WHERE f.owner_id=${uid} AND f.archived=false AND r.row_key IS NOT NULL
          ${type == null ? db`` : db`AND COALESCE(f.data->>'type', '')=${type}`}
          GROUP BY COALESCE(f.data->>'type', ''), r.row_key
        )
        SELECT g.k AS k, g.entries AS entries FROM groups g JOIN type_counts t ON t.type=g.type WHERE t.n>1 AND g.n>1`;
      return rows.map((r: any) => ({ k: String(r.k), entries: json(r.entries) as { fileId: string; fileName: string; rowIdx: number }[] }));
    }
    ...
    case "walletWithdrawals": { const uid = String(a.uid || ""); if (!uid) throw new Error("uid required"); return db`SELECT id,user_id,amount::float8 AS amount,method,account,status,created_at::float8 AS created_at,updated_at::float8 AS updated_at FROM withdrawals WHERE user_id=${uid} ORDER BY created_at DESC LIMIT 500`; }
    ...
    case "metaGet": { const k = String(a.k || ""), cached = await redisJsonGet(`ss:meta:${k}`); if (cached !== undefined) return cached; const r: any = (await db`SELECT v FROM meta WHERE k=${k}`)[0]; const value = json(r?.v) ?? null; if (r) void redisJsonSet(`ss:meta:${k}`, value, k.startsWith("check:") ? 60 : 60); return value; }
...
async function fileOp(id: string, op: string, a: any) {
  ...
  if (["keys", "projection"].includes(op)) { const limit = Math.min(10000, Math.max(1, Number(a.limit) || 10000)); const krows: any[] = await db`SELECT idx AS i,row_key AS k FROM file_rows WHERE file_id=${id} AND row_key IS NOT NULL ORDER BY idx LIMIT ${limit}`; return krows.map((r: any) => ({ k: String(r.k), i: Number(r.i) })); }
...
async function poolOp(password: string, op: string, a: any) {
  ...
  // dead rows are unconsumed husks — removed on re-feed so the account can be pooled again.
    // an existing AVAILABLE row gets its data refreshed from the newest feed (fresh check_status/cookie edits); held/claimed stay frozen.
  ...
  if (op === "add") return db.begin(async (tx: any) => {
    ...
    // check_status from the per-owner check cache (check:{srcUid}:{cuser}, fresh +
    // eligible only; legacy wa: keys read as fallback) — never overwrite an explicit value already on the row.
    if (srcUid && cand.size) {
      const need = [...cand].filter(([, v]) => !String((v.data as any)?.check_status ?? (v.data as any)?.wa_status ?? (v.data as any)?.waStatus ?? "").trim());
      if (need.length) {
        const mkeys = [...new Set(need.flatMap(([k]) => [`check:${srcUid}:${k}`, `wa:${srcUid}:${k}`]))];
        ...
        for (const [k, v] of need) {
          const ce: any = mc.get(`check:${srcUid}:${k}`) ?? mc.get(`wa:${srcUid}:${k}`);
          if (!ce || ce.status !== "eligible" || (ce.ts && now - Number(ce.ts) > 86400000)) continue;
          const d = v.data as any;
          d.check_status = "eligible";
          if (ce.banReason != null) d.check_ban_reason = ce.banReason;
          if (ce.pageName != null) d.check_page_name = ce.pageName;
          if (ce.linkedNumber != null) d.check_linked_number = ce.linkedNumber;
        }
      }
    }
    ...
    if (keys.length) {
      ...
      // lost check_status keeps the stored eligible flags instead.
      if (touch.length) {
        const stored = new Map(same.map((r: any) => [`${r.pool_id} ${r.row_key}`, json(r.data)]));
        for (const t of touch) {
          const d = t.d as any;
          if (String(d?.check_status ?? d?.wa_status ?? d?.waStatus ?? "").trim()) continue;
          const sd: any = stored.get(`${t.pool} ${t.k}`);
          if (String(sd?.check_status ?? sd?.wa_status ?? sd?.waStatus ?? "").toLowerCase() !== "eligible") continue;
          d.check_status = "eligible";
          if (d.check_ban_reason == null && sd.check_ban_reason != null) d.check_ban_reason = sd.check_ban_reason;
          else if (d.check_ban_reason == null && sd.wa_ban_reason != null) d.check_ban_reason = sd.wa_ban_reason;
          if (d.check_page_name == null && sd.check_page_name != null) d.check_page_name = sd.check_page_name;
          else if (d.check_page_name == null && sd.wa_page_name != null) d.check_page_name = sd.wa_page_name;
          if (d.check_linked_number == null && sd.check_linked_number != null) d.check_linked_number = sd.check_linked_number;
          else if (d.check_linked_number == null && sd.wa_linked_number != null) d.check_linked_number = sd.wa_linked_number;
        }
      }
  ...
  if (op === "diag") {
    ...
    const found = fr.map((x: any) => {
      ...
      return {
        ...
        check: String((d as any).check_status ?? (d as any).wa_status ?? (d as any).waStatus ?? ""), key: kk, live: liveRow(d), pool: classify(d, pp),
      };
    });
  ...
  if (op === "rows") {
    ...
    const eligibleFilter = a.verifiedOnly ? db`AND wa_eligible` : a.unverifiedOnly ? db`AND NOT wa_eligible` : db``;
  ...
  if (op === "verifiedCounts") { const total: any = (await db`SELECT COUNT(*) n, COUNT(*) FILTER (WHERE wa_eligible) v FROM pool_rows WHERE password=${password} AND pool_id=${p} AND state='available' AND (src_file_id IS NULL OR EXISTS (SELECT 1 FROM file_index WHERE file_id=pool_rows.src_file_id AND archived=false))`)[0]; const totalN = Number(total.n), verified = Number(total.v); return { pool: p, verified, unverified: totalN - verified, totalAvailable: totalN, ...(p === "page" ? { totalCookies2faAvailable: totalN, unverifiedScanned: totalN } : {}), truncated: false, scanCap: totalN }; }
  if (op === "downloads" || op === "holds") { const lim = Math.min(200, Math.max(1, Number(a.limit) || 50)); const off = Math.max(0, Number(a.offset) || 0); const where = op === "holds" ? a.status ? db`AND status=${String(a.status).toUpperCase() === "PENDING" ? "HOLD" : String(a.status).toUpperCase()}` : db`AND status IN ('HOLD','APPROVED')` : db``; const rows = await db`SELECT * FROM downloads WHERE password=${password} ${where} ORDER BY ts DESC LIMIT ${lim} OFFSET ${off}`; const out = rows.map(downloadShape); return op === "holds" ? { holds: out, downloads: out } : { downloads: out }; }
  ...
  if (op === "holdsAll" || op === "downloadsAll") { const lim = Math.min(200, Math.max(1, Number(a.limit) || 50)); const off = Math.max(0, Number(a.offset) || 0); const where = op === "holdsAll" ? (a.status ? db`WHERE status=${String(a.status).toUpperCase() === "PENDING" ? "HOLD" : String(a.status).toUpperCase()}` : db`WHERE status IN ('HOLD','APPROVED')`) : db``; const rows: any[] = await db`SELECT * FROM downloads ${where} ORDER BY ts DESC LIMIT ${lim} OFFSET ${off}`; const out = rows.map((r: any) => ({ ...downloadShape(r), password: r.password })); return op === "holdsAll" ? { holds: out, downloads: out } : { downloads: out }; }
...
async function allocate(password: string, op: string, pool: string, a: any) {
  ...
  return db.begin(async (tx: any) => {
    const did = String(a.downloadId || ""); if (did) { const ex: any = (await tx`SELECT * FROM downloads WHERE id=${did}`)[0]; if (ex) { if (ex.password !== password || String(ex.claimed_by) !== String(a.uid)) throw new Error("request id conflict"); const unit0 = ex.unit_price == null ? price(ex.pool_id) : Number(ex.unit_price); const claimed0 = Number(ex.claimed); const total0 = ex.total == null ? +(unit0 * claimed0).toFixed(2) : Number(ex.total); const rows0 = json(ex.rows); if (op === "hold") return { claimed: claimed0, held: claimed0, count: claimed0, rows: rows0, holdId: ex.id, downloadId: ex.id, filename: ex.filename, status: ex.status, unitPrice: unit0, total: total0, mode: ex.mode || "fifo", srcUids: json(ex.src_uids), srcFileIds: json(ex.src_file_ids) }; return { claimed: claimed0, rows: rows0, downloadId: ex.id, filename: ex.filename, status: ex.status, unitPrice: unit0, total: total0, mode: ex.mode || "fifo" }; } }
    ...
    const elig = a.verifiedOnly ? tx`AND wa_eligible` : a.unverifiedOnly ? tx`AND NOT wa_eligible` : tx``;
