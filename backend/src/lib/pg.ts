...
async function poolOp(password: string, op: string, a: any) {
  ...
  if (op === "summaryAll") { const rows: any[] = await db`SELECT password,pool_id,COUNT(*) FILTER (WHERE state='available') available,COUNT(*) FILTER (WHERE state='claimed') claimed,COUNT(DISTINCT src_uid) users FROM pool_rows GROUP BY password,pool_id`; const rej: any[] = await db`SELECT password,pool_id,COUNT(*) n FROM pool_rejects GROUP BY password,pool_id`; const m = new Map<string, any>(); for (const r of rows) m.set(JSON.stringify([r.password, r.pool_id]), { password: r.password, pool: r.pool_id, available: Number(r.available), claimed: Number(r.claimed), users: Number(r.users), invalid: 0 }); for (const x of rej) { const k = JSON.stringify([x.password, x.pool_id]); const e = m.get(k) || { password: x.password, pool: x.pool_id, available: 0, claimed: 0, users: 0, invalid: 0 }; e.invalid = Number(x.n); m.set(k, e); } return [...m.values()]; }
...
async function allocate(password: string, op: string, pool: string, a: any) {
  ...
  return db.begin(async (tx: any) => {
    ...
    const unit = Number(((await tx`SELECT price FROM pool_settings WHERE password=${password} AND pool_id=${pool}`)[0]?.price ?? price(pool)));
